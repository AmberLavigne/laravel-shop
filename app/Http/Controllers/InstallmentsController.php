...
class InstallmentsController extends Controller
{
    ...
    public function alipayReturn(){
        ...
        $this->selfAlipayNotify($data);
        return view('pages.success', ['msg' => '付款成功']);
    }
    public function selfAlipayNotify($data)
    {
        ...
        $data_trade_no = Installment::findAvailableNo();
        // 使用事务，保证数据一致性
        \DB::transaction(function () use ($data_trade_no, $no, $installment, $item) {
            // 更新对应的还款计划
            $item->update([
                'paid_at'        => Carbon::now(), // 支付时间
                'payment_method' => 'alipay', // 支付方式
                'payment_no'     => $data_trade_no, // 支付宝订单号
            ]);
